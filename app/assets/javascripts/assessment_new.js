$(document).on('ready', function() {
  ...
  function updateGroupProgressCounter($group){
    var count = $group.find('tr').has('.select .checked').length;
  ...
  function isSubGroupValid($subGroup){
    ...
    var isValid = true;
    $subGroup.find('select').each(function(){
      var n = $(this).attr('name'),
          $uncheckedRadioSet = $subGroup.find('select[name="'+n+'"]:not(:selected)');
  ...
  $(window).on('beforeunload', function(){
    return 'If you leave you will need to restart the assessment.';
  });
});
